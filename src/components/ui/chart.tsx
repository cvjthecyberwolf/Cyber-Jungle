...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  const sanitizeColor = (value?: string) => {
    ...
    if (/^var\(--[a-zA-Z0-9_-]+\)$/.test(trimmed) || /^[a-zA-Z][a-zA-Z0-9-]*$/.test(trimmed)) return trimmed
    return ''
  }
    ...
    .map(([theme, prefix]) => {
      const lines = colorConfig
        .map(([key, itemConfig]) => {
          ...
          const color =
            (itemConfig.theme?.[theme as keyof typeof itemConfig.theme] as string | undefined) ||
